    ...
    fn utility_commands_roundtrip_inventory_idempotency_delete() {
        for client in ["claude", "cursor", "antigravity"] {
            ...
            for (cmd, path) in utility_artifact_paths(client, &dir) {
                ...
                if cmd == "memorize" {
                    // Sprint 28f Stage 6: THE DIRECTION IS REVERSED, and these three
                    // assertions are reversed with it.
                    //
                    // 28c pinned the opposite, for a reason true of the engine at the
                    // time: the store was rebuilt from a file substrate, so a direct
                    // record wrote a row nothing could put back, and the text had to
                    // send agents to author a file. Stage 1 gave the store a backup
                    // before every destructive verb, Stage 2 made its writes durable,
                    // and Stage 6 makes an acceptance write the story file itself. The
                    // database is the truth now and the folder is its export.
                    assert!(
                        body.contains("kind=record") && body.contains("kind=promote"),
                        "{client}: /memorize must send agents to RECORD and then to \
                         ACCEPT — those two verbs are what make knowledge durable and \
                         what writes the story file; an agent authoring markdown by \
                         hand is the model this sprint replaced"
                    );
                    assert!(
                        !body.contains("substrate.root"),
                        "{client}: /memorize still tells agents to ask for a substrate \
                         root and write a file under it. That was right while the store \
                         was rebuilt FROM files; it now instructs every agent to \
                         hand-author what the store writes for them"
                    );
                    // THE STAMP IS THE STORE'S TO WRITE, AND THE TEXT MUST NOT CLAIM IT.
                    // `reviewed:` is the one field the reseed gate trusts, so a field the
                    // gate trusts must not be authored by the thing it gates. Since the
                    // acceptance stamps the row, an agent following this text CANNOT
                    // forge a review — and the text must not invite it to try.
                    assert!(
                        !body.contains("`reviewed:` with today's date"),
                        "{client}: /memorize still tells agents to write the review stamp \
                         themselves. The store stamps it at the acceptance now, which is \
                         what makes forging one impossible rather than merely forbidden"
                    );
                    assert!(
                        body.contains("FULL TEXT"),
                        "{client}: /memorize must echo what it stored in full — this \
                         command stored one compressed line for months, and the only \
                         reason that went unnoticed is that its report never showed it"
                    );
                    // Sprint 28c: this text is a PRODUCER of experience records —
                    // it is the instruction agents follow. The engine's form gate
                    // refuses a lesson or failure_mode with no situation and no
                    // outcome, so an instruction that never mentions them sends
                    // every agent into a refusal. Deploying the text is the fix;
                    // this asserts the deployed copy carries it.
                    assert!(
                        body.contains("situation") && body.contains("verdict"),
                        "{client}: /memorize must tell agents what an experience owes"
                    );
                    assert!(
                    ...
                    assert!(
                        ...
                        "{client}: /memorize must also say a domain_fact owes no outcome — \
                         without that half, agents invent verdicts for facts and \
                         retrieval ranks on fiction"
                    );
                }
