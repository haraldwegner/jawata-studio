                ...
                if cmd == "memorize" {
                    ...
                    assert!(
                        ...
                        "{client}: the deployed /memorize still says STORE FIRST, which \
                         now instructs every agent to write something the next reseed \
                         deletes"
                    );
                    assert!(
                        body.contains("substrate.root"),
                        "{client}: /memorize must ASK the store where the substrate is; \
                         a path an agent invents is the same failure as a value invented \
                         to satisfy a rule"
                    );
                    assert!(
                        body.contains("reviewed:"),
                        "{client}: /memorize must write the reviewed stamp the reseed \
                         gate requires, or every story it writes is refused"
                    );
                    assert!(
